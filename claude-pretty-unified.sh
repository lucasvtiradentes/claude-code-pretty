...
process_user_message() {
  local line="$1"
  local content_is_array content_type
  content_is_array=$(echo "$line" | jq -r 'if .message.content | type == "array" then "yes" else "no" end')

  if [[ "$content_is_array" == "no" ]]; then
    if [[ "$MODE" == "replay" ]]; then
      local text
      text=$(echo "$line" | jq -r '.message.content // empty')
      if [[ -n "$text" ]]; then
        echo -e "\n${GREEN}[user]${RESET} ${text:0:200}"
        [[ ${#text} -gt 200 ]] && echo -e "${DIM}...${RESET}"
      fi
    fi
    return
  fi
...
process_assistant_message() {
  ...
  if [[ "$MODE" == "replay" ]]; then
    if echo "$line" | jq -e '.message.content[]? | select(.type == "text")' &>/dev/null; then
      local text
...
show_help() {
  echo "Usage: claude-pretty-unified.sh [-f FILE] [PROMPT...]"
  echo ""
  echo "Options:"
  echo "  -f FILE    Replay a saved session .jsonl file"
  echo "  -h         Show this help"
  echo ""
  echo "Examples:"
  echo "  claude-pretty-unified.sh \"explain this code\""
  echo "  claude-pretty-unified.sh -f ~/.claude/projects/.../session.jsonl"
}
...
if [[ "${1:-}" == "-f" ]]; then
  ...
  MODE="replay"
  echo -e "${DIM}[replay] $2${RESET}\n"
  export -f process_json process_user_message process_assistant_message update_sp
  export GREEN ORANGE PURPLE CYAN BLUE YELLOW DIM RED RESET INDENT FILE_LINES RESULT_LIMIT HIDE_TOOLS MODE SP SUBAGENT_DEPTH
  awk '{print NR"\t"$0}' "$2" | while IFS=$'\t' read -r _ line; do
